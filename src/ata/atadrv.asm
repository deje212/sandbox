...
; Where this comes from? Partition Table?
struc drivedatastruc
.dd_stLBA:  resd  1     ; Disk Start LBA (28?).
.dd_prtlen: resd  1     ; Partition Length in sectors.
.dd_dcr:    resw  1     ; ? (status port?)
.dd_tf:     resw  1     ; ? (I/O port base?)
.dd_sbits:  resb  1     ; ? (flags?)
endstruc

; do a singletasking PIO ATA read
;
; inputs: ebx = # of sectors to read, 
;         edi -> dest buffer, 
;         esi -> driverdata struct, 
;         ebp = 4b LBA
...
read_ata_st:
  ...
  mov   edx, [esi + drivedatastruc.dd_prtlen]  ; get the total partition length (sectors)
  dec   edx                     ; (to avoid handling "equality" case)
  cmp   edx, ebp                ; verify ebp is legal (within partition limit)
  jb    .r_don                  ; (carry is set automatically on an error)
  ...
  mov   dx, [esi + drivedatastruc.dd_dcr]      ; dx = alt status/DCR
...
; if ebp > 28bit or esi.stLBA > 28bit or stLBA+ebp > 28bit or stLBA+ebp+ebx > 28bit, use 48 bit
  ...
  mov   eax, [esi + drivedatastruc.dd_stLBA]
  cmp   eax, 0xfffffff
  jg    .setreg
  ...
  cmp   eax, 0xfffffff
.setreg:
  mov   dx, [esi + drivedatastruc.dd_tf]         ; dx = IO port base ("task file")
  jle   .read28                   ; test the flags from the eax cmp's above
...
.read48:
  ...
  je    .read48                   ; if successful, is there more to read?
  jmp   .r_don
...
.read28:
  ...
  je    .read28                   ; if successful, is there more to read?
...
;
; inputs: ESI -> driverdata info, 
;         EDI -> destination buffer
;         BL = sectors to read, 
;         DX = base bus I/O port (0x1F0, 0x170, ...), 
...
pio28_read:
  add   ebp, [esi + drivedatastruc.dd_stLBA]  ; convert relative LBA to absolute LBA
  ...
  or    al, [esi + drivedatastruc.dd_sbits]  ; master/slave flag | 0xe0
...
.pior_l:
  in    al, dx      ; grab a status byte
  test  al, 0x80    ; BSY flag set?
  jne   .pior_l     ; (all other flags are meaningless if BSY is set)
...
; receive and ack the IRQ -- or poll the status port all over again
  ...
  sub   ebp, [esi + drivedatastruc.dd_stLBA] ; convert absolute lba back to relative
...
;
; inputs: bx = sectors to read (0 means 64K sectors), 
;         edi -> destination buffer
;         esi -> driverdata info, 
;         dx = base bus I/O port (0x1F0, 0x170, ...), 
;         ebp = 32bit "relative" LBA
; BSY and DRQ ATA status bits must already be known to be clear on both slave and master
; outputs: data stored in edi; edi and ebp advanced, ebx decremented
; flags: on success Zero flag set, Carry clear
pio48_read:
...
;outb (0x1F5, LBA3)
  ...
  mov   al, byte [esi + drivedatastruc.dd_sbits] ; master/slave flag | 0xe0
  inc   edx
